...
void CNamedPipeCltView::OnPipeConnect() 
{
	...
	if(!WaitNamedPipe("\\\\.\\pipe\\SunxinPipe", NMPWAIT_WAIT_FOREVER))
    {
        MessageBox("当前没有可利用的命名管道实例!"); 
        return; 
    }

    hPipe = CreateFile("\\\\.\\Pipe\\SunxinPipe", GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL); 
