...
BOOL CChatDlg::OnInitDialog()
{
	...
	if(InitSocket())
    {
        RECVPARAM* pParam = new RECVPARAM(); 
        pParam->sock = m_socket; 
        pParam->hwnd = m_hWnd; 
...
BOOL CChatDlg::InitSocket()
{
    ...
    //m_socketSend = socket(AF_INET, SOCK_DGRAM, 0); 
...
{
    RECVPARAM* pParam = (RECVPARAM*)lpParam; 
    ...
    while(TRUE)
    {
        retval = recvfrom(pParam->sock, recvBuf, 200, 0, (SOCKADDR*)&addrFrom, &len); 
        if(retval == SOCKET_ERROR)
        {
            //Sleep(1000); 
            continue; //break; 
        ...
        sprintf(tempBuf, "%s says: %s", inet_ntoa(addrFrom.sin_addr), recvBuf); 
        ::PostMessage(pParam->hwnd, WM_RECVDATA, 0, (LPARAM)tempBuf); 
    }
...
void CChatDlg::OnBtnSend() 
{
    ...
    //sendto(m_socketSend, strSend, strSend.GetLength()+1, 0, (SOCKADDR*)&addrTo, sizeof(SOCKADDR)); 
