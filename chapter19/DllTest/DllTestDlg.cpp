...
#include "..\DllPlus\DllPlus.h"

typedef int (_stdcall *ADDPROC)(int a, int b); 
typedef int (_stdcall *MINUSPROC)(int a, int b); 
void CDllTestDlg::OnBtnAdd() 
{
    ...
    HINSTANCE hInst = LoadLibrary("DllPlus.dll"); 
    ADDPROC add = (ADDPROC)GetProcAddress(hInst, "_add@8"); 
    //HINSTANCE hInst = LoadLibrary("DllMinus.dll"); 
...
void CDllTestDlg::OnBtnSubtract() 
{
    ...
    HINSTANCE hInst = LoadLibrary("DllMinus.dll"); 
    MINUSPROC minus = (MINUSPROC)GetProcAddress(hInst, MAKEINTRESOURCE(2)); 
    if(minus != NULL)
    {
        CString str; 
        str.Format("5-3=%d", minus(5, 3)); 
        MessageBox(str); 
    }
    else 
        MessageBox("获取函数地址失败!"); 
